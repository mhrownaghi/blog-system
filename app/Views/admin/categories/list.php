...
<div class="toolbar">
    <div class="search">
        <form action="<?= current_url() ?>" method="get">
            ...
            <a class="btn" href="/admin/categories">حذف فیلتر</a>
        </form>
    </div>
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th class="w-20">
                    <a href="<?= current_url() . '?' . ($search ? 'search=' . $search . '&' : '') . 'order=note' . ($dir === 'ASC' && $order === 'note' ? '&dir=DESC' : '&dir=ASC') ?>">
                        یادداشت
                    </a>
                    <?php if ($order === 'note') : ?>
                        <?php if ($dir === 'ASC') : ?>
        ...
        <tbody>
            <?php foreach ($categories as $category) : ?>
                <tr class="data">
                    ...
                    <td><?= $category->note ?></td>
                    <td>
                        <?php $date_array = date_parse($category->created_at) ?>
                        <?= $cal->gregorianToJalali($date_array['year'], $date_array['month'], $date_array['day'], true) ?>
                    </td>
                </tr>
                <tr class="actions">
                    <td colspan="4">
                        <a href="/admin/categories/edit/<?= $category->id ?>"><i class="nf nf-cod-edit"></i>ویرایش</a>
                        <a href="/admin/categories/remove/<?= $category->id ?>"><i class="nf nf-mdi-delete_forever"></i>حذف</a>
                    </td>
                </tr>
            <?php endforeach ?>
        </tbody>
